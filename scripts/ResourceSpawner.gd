...
@export var branch_from : float = 0.0
@export var branch_to : float = 1.0
@export var tree_from : float = 0.0
@export var tree_to : float = 1.0
@export var forest_from : float = 0.0
@export var forest_to : float = 0.0
@export var pebble_from : float = 0.0
@export var pebble_to : float = 1.0
@export var boulder_from : float = 0.0
@export var boulder_to : float = 1.0
...
func _spawn_resource(resource, resource_radius, from_percent, to_percent):
	...
	var resource_count: int = randf_range(from_percent, to_percent) * full_resource
	for i in range(0,resource_count):
		var angle = randf_range(spawn_angle_start, spawn_angle_end)
		var dist = randi_range(spawn_radius_start, spawn_radius_end)
		var resource_instance = resource.instantiate()
		resource_instance.global_position.x = 32 * dist * cos(angle)
		resource_instance.global_position.y = 32 * dist * sin(angle)
